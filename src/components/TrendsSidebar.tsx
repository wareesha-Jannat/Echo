...
async function WhoToFollow(){
  ...
  const usersToFollow = await prisma.user.findMany({
    ...
    return <div className='space-y-5 rounded-2xl bg-card shadow-sm p-5'>
...
`
  ...
  async function TrendingTopics (){
    ...
    return <div className='space-y-5 rounded-2xl bg-card p-5 shadow-sm'>
     ...
     return <Link key={title} href={`/search?q=${encodeURIComponent("#" + title)}`} className='block'>
      <p className=' break-all font-semibold hover:underline' title={hashtag}>
{hashtag}
      </p>
