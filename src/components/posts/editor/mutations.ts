...
export function useSubmitPostMutation(){
    ...
    const mutation = useMutation({
        ...
        const queryKey  = ["post-feed", "for-you"]
                            ...
                            pages:[
                                {
                                    posts:[newPost, ...firstPage.posts],
                                    nextCursor: firstPage.nextCursor,
                                    moods: newPost.mood !== null ? [...firstPage.moods, newPost.mood] : [...firstPage.moods]
                                },
                                ...oldData.pages.slice(1),
                            ]
                        }
            ...
            queryClient.invalidateQueries({  //if user tries to post something before existing posts load 
                queryKey: queryFilter.queryKey,
                predicate(query) {
                    return queryFilter.predicate(query) && query.queryKey.includes("for-you") && !!query.state.data;
                },
            })
            toast({
