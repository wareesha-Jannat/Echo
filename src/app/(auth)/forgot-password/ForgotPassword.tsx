...
const ForgotPassword = () => {
 ...
 const mutation = useMutation({
  ...
  return (
    <div>
      <Form {...form}>
        <form onSubmit={form.handleSubmit((values) => mutation.mutate(values))} className='space-y-3 w-full'>
            ...
            render={({field}) => (
                <FormItem>
                    ...
                    <FormControl>
                        <Input placeholder='enter your email' {...field} type='email' className='mt-2' />
                    </FormControl>
                    <FormMessage />
                </FormItem>
             )}
             ...
             <LoadingButton disabled={mutation.isPending} type='submit' loading={mutation.isPending}  >{mutation.isPending ? "Sending Link " : "Send Link " }</LoadingButton>
        </form>
